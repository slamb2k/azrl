...
- **`internal/ui/`** — tabbed Bubble Tea TUI. `tabsModel` container: winged banner (blank line below), tab strip `Dashboard | Azure | GitHub | AWS | Google Cloud` (`preferredOrder`; the `PROVIDERS` key in `azrl.conf` — options popup `o`, default `azure,github` — selects which appear), `⇥`/`⇧⇥`/`[`/`]` cycle tabs, `↑` at the top of any list focuses the tab bar (`←`/`→` pick, `↓` returns), `d` opens a fuzzy change-directory picker (`dirpicker.go`; os.Chdir + `cwdChangedMsg` broadcast), `o` a centered options popup (`options.go`, ANSI-overlay via `overlayCenter`); mouse support via bubblezone v1 (`zone.Scan` at the root `tabsModel.View` only, zones `tab:`/`prof:`/`act:`/`dash:`/`box:` — a click selects, click-again runs; the wheel moves the focused list's cursor; overlays swallow mouse input and dismiss on an outside click; the tab-cell click loop is gated by the same `activeCapturesInput()` check as keyboard tab-switching, so a click can't steal focus out of a naming/picker prompt). One selection language: bright `selBlockActive` in the focused level, dim `selBlockParent` on ancestors only (`selMode`), tri-level focus tab bar → PROFILES → DETAILS/ACTIONS. Every tab is one implementation — the shared `providerTabView` (`provider_view.go`); Azure is a thin `azureView` wrapper (`azure_view.go`) adding the live drift notice and the `e` write-.envrc hotkey, and `frame.go` holds the shared pane renderers. Every tab shares the header anatomy (`headerStrip`: provider icon `providerIcon` · 📁 cwd · 👤 effective identity via `effectiveIdentity`, plus an optional notice line) and pane frame (`renderPaneFrame`, legend `scopeLegend` bottom-anchored via the left-footer slot); when `$AZRL_PROFILE` matches the tab's provider the header instead shows `⌁ shell: <name>` (Azure's drift notice goes quiet since the shell chip already says which identity governs this dir). Profile rows (`renderProfilePane`) lead with a relevance mark meaning exactly one thing — ● green linked in cwd / ● orange linked via a parent / empty otherwise — with the ambient default carrying a trailing `⌁ default` tag; the most-active profile is bold, renamed labels italic. The DETAILS pane shows a key/value sheet (`profileInfoBlock`: Name/Identity/Detail/Browser/Linked/Expiry/Last used/Drift) over an `ACTIONS (n)` radio with a **never-hide action model** (`enabledActions`): every verb always listed, inapplicable ones disabled with the reason as their hint. Tabs are persona-only — the everyday ACTIONS radio is exactly 5, never more: `s` Renew (sign the profile in again — links unchanged; visible even with a live session), `t` Shell as… (subshell as the selected profile via `runShellHandoff`, no link), `c` Open console (web console as the selected credential via `consoleAction`, no session mutation), `b` Assign browser… (async discovery + fuzzy overlay picker `browserpicker.go`, manual-entry fallback), `delete` Delete… (confirm dialog everywhere — link-aware, see below). `u` Link here left the tabs entirely — it lives only on the dashboard (`u` link / `U` unlink); `n` New profile left the ACTIONS radio for the `NEW ＋` button right-aligned in the PROFILES title bar (zone `prof:+new` — a button, not a list row: `v.cursor` indexes `v.profiles` directly and only ever lands on real profiles). Clicking `NEW ＋`, or `n` from anywhere on the tab (footer chip `n · new profile`), opens the naming prompt (hint `token container + sign-in — link it later`, intro blurb `A profile is a container for tokens and intention — sign in once, link it to any number of directories.`) and execs `login <name> --yes --no-link` — create without claiming the directory. `a` Capture (empty-state onboarding only, name-prompted; empty-state ACTIONS = 1, Capture only), `r`/`f5` refresh, `?` full-keymap overlay (container-level), `e` write .envrc (Azure). Delete on a profile that's still linked lists the linked directories (first 3 + `+ n more`) and offers a radio: `Cancel` / `Unlink N dir(s) + delete` (`Scheme.UnlinkAll` then `Remove`) / `Replace links with…` (a picker over the provider's other profiles; `Scheme.ReplaceLinks` then `Remove`, disabled `no other profile to point them at` when none exist); a link-free profile keeps the simple two-option confirm. Edit/Rename retired from the TUI (edit the .conf directly). All footers use `keyHelp` chips. The dashboard (`overview.go`'s `BuildOverview`, shared with `cmd status`) is live via fsnotify + `DASHBOARD_POLL_SECS` tick; its header carries a prioritised next-action hint (`dashboardHint`); its cursor starts on the cwd-governing row. Dashboard rows take the verb keymap directly, no tab switch needed — `s`/`t`/`c` exec the provider's login/shell/console handoff, `u` links the row's profile in-process (`Provider.Use`), `U` unlinks in-process (`Scheme.Unlink`): a mapping row whose Dir is the cwd unlinks with status `✓ unlinked <dir> (profile kept)`, a mapping row for another directory explains `links are removed where they live — run unlink in <dir>`, and an ambient/unmapped row says `no directory link on this row`; `b` routes to the row's own tab via `switchTabMsg.action` for its browser-picker flow, and a status line under the table echoes the result. Adoptable rows (unmanaged GitHub git-config mappings and unmanaged AMBIENT defaults on any provider) take `a`, which prompts for a name (prefilled `profile.DefaultName`) before exec-ing the provider's `capture`. Expiry is guidance, not telemetry: only AWS rows show markers (amber `⚠ expires in …` inside 15 min, `⚠ expired` after; MAPPINGS + managed AMBIENT rows only, and the expired dashboard hint + plain-status note are AWS-gated via `ui.ExpiryActionable`); Azure/GCP rows never show expiry — the DETAILS pane says `token stale · refreshes on next use` (their CLIs refresh silently); GitHub shows nothing. UNMAPPED rows carry no expiry. Interactive flows (sign in, new profile, adopt) exec the real CLI via `runHandoff` and reload on `opDoneMsg`. `ghrl` opens on the GitHub tab.
