...
- **`cmd/`** — Cobra tree. Azure at top level (`login`, `capture`, `use`, `unmap`, `rm`, `list`, `shell`, `console`); `login <name>` creates a profile on first use (Azure discovers the tenant via a tenant-less sign-in; `--yes` skips the confirm) and pin-on-create writes the dir pointer uniformly across providers (existing-profile login never pins) — the old `init` command was removed (a hidden deprecated `init` stub just points users at `login`); `login`/`gh|aws|gcp login <new-name>` accept `--no-map` (legacy alias `--no-link`) to create the profile without writing the dir pointer (the CLI default for users not passing the flag is unchanged — map on create); `unmap` (alias `unlink`; `cmd/unlink.go`, registered on all four surfaces via `newUnlinkCmd`) removes only the calling directory's own pointer + mapping row, profile and tokens untouched — governed-by-a-parent refuses naming the parent (`azrl: this directory is governed by <dir>/<pointer> (profile <name>) — run unmap there`), nothing-mapped refuses too; `rm` (and the three group `rm`s) refuse to delete a profile while any directory still maps to it unless `--unmap-all` (legacy alias `--unlink-all`; `Scheme.UnlinkAll` first) or `--replace <name>` (`Scheme.ReplaceLinks` — validated target profile, rewrites each linked dir's pointer + mapping row) is passed, mutually exclusive flags; cross-provider `status [--json]` (disk-only, three sections: MAPPINGS / AMBIENT / UNMAPPED PROFILES via `ui.BuildOverview`; `--json` emits `{"mappings":[…],"ambient":[…],"unmapped":[…]}`, plus an omitempty top-level `shell_override` mirroring `$AZRL_PROFILE` — reshaped in v0.7.0); a `gh` group (`gh login/list/use/unmap/rm/capture/status/browser/shell/console` — `switch` was removed; a hidden deprecated `switch` stub on both the gh group and the promoted ghrl top level just points users at `gh auth switch` / `use`, like the `init` stub); an `aws` group (`aws login/list/use/unmap/rm/capture/status/browser/shell/console` — no `switch` verb, self-wired via `init()`); a `gcp` group (`gcp login/list/use/unmap/rm/capture/status/browser/shell/console`, likewise self-wired via `init()`); a top-level `setup` command (`cmd/setup.go`: env-detect wizard writing `azrl.conf`; `--yes`/`--print`); a top-level `browser <name>` verb (Azure); `browser` maps a profile to a local Edge/Chrome profile discovered via `internal/browserpick` (numbered pick, identity-match sorted, manual entry, clear); hidden `__browser` (smart shim) and `__browser-capture` self-shims. `capture` derives missing fields from the ambient session when flags are omitted (azure: `az account show`; github: falls back to the ambient gh session when the isolated dir has no session; aws: SSO fields from the ambient config stanza incl. `sso-session` indirection; gcp: binds the ambient active configuration's name/project/region). `shell <name>` opens `$SHELL` as the profile (sign-in first when the session is dead via a child `login`, env map = the .envrc values + `AZRL_PROFILE=<provider>:<name>` + the profile's `AZRL_BROWSER_CMD`; exit status passes through; nested shells warn, innermost wins). `console <name>` (`cmd/console.go`) opens the provider's web console as that profile's credential — deep links built from conf data already on disk (azure `#@<Tenant, else TenantID>`, aws the profile's `AWS_SSO_START_URL` verbatim, gcp project + `authuser` from the disk-only `Status().Identity`, github `https://<GH_HOST>`) via a profile `*_BROWSER_CMD` override falling back to the global `BROWSER_CMD`; every degraded path (no browser configured, launch failure) prints the URL and returns nil — never an error, only missing profile data (no tenant/start URL/project/host) does. Bare `azrl` launches the `internal/ui` tabbed TUI (dashboard is the default landing view).
...
- `AZRL_PROFILE` — the `<provider>:<name>` marker `azrl shell`/`gh|aws|gcp shell`/`ghrl shell` sets on its subshell; read by the TUI header chip and `azrl status`'s `shell_override`, never persisted to disk.
