> **⚠️ Historical document.** Written during the original build, when the tool
> was named **`azlogin`** and lived at `~/.azure-profiles/azlogin/`. The project
> has since been renamed to **`azrl`** (Azure Remote Login) and moved to
> `~/work/azrl/` with its own GitHub repo. Internal references to `azlogin`,
> `azl_*`, `AZL_*`, and the old path are therefore stale. Kept for design
> provenance — see `README.md` and `HANDOVER.md` for current state.
